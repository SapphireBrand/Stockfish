          ...
          if (newDepth == 2) {
              ...
              if (captureOrPromotion) // 42490 hits in bench
                  ...
                  if (PvNode) { // 14508 hits in bench
                      ...
                      else if (lastHistory <= pvHiNonHiCutoff && type_of(move) == NORMAL && !pos.see_ge(make_move(to_sq(move), from_sq(move))))
                            historyCutoff = pvHiNonHilo;    // 38 hits in bench. Deleting this line: -0.0126, which seems very large
                  }
                  else if (cutNode) {
