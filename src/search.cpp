...
int paramReductionFactor[4] = { 1932, 1929, 1964, 1942, };
int paramFutilityConstant[4] = { 2406, 5000, 5000, 4976, };
int paramFutilityPowMultipier[4] = { 739, 991, 986, 1007, };
int paramFutilityPowExponent[4] = { 1808, 1981, 1981, 2022, };

TUNE(SetRange(1842, 2042), paramReductionFactor[3], Search::init);
TUNE(SetRange(4876, 5076), paramFutilityConstant[3], Search::init);
TUNE(SetRange(907, 1107), paramFutilityPowMultipier[3], Search::init);
TUNE(SetRange(1922, 2122), paramFutilityPowExponent[3], Search::init);
...
void Search::init() {
  ...
  for (int su = SituationNormal; su <= SituationImproving; ++su)
      for (int d = 1; d < 64; ++d)
          ...
          {
              double r = log(d) * log(mc) / (paramReductionFactor[su] / 1000.0);
  ...
  for (int su = SituationNormal; su <= SituationImproving; ++su)
      for (int d = 0; d < 16; ++d)
          FutilityMoveCounts[su][d] = int(paramFutilityConstant[su] / 1000.0 + (paramFutilityPowMultipier[su]/1000.0) * pow(d, paramFutilityPowExponent[su] / 1000.0));
}
