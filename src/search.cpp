    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || cutNode))
      {
          ...
          if (!captureOrPromotion)
          {
              ...
              if (!inCheck && !givesCheck && pos.non_pawn_material() > 2 * (BishopValueEg + RookValueEg + QueenValueEg) && to_sq(move) == from_sq((ss - 2)->currentMove) && from_sq(move) == to_sq((ss - 2)->currentMove))
                r += ONE_PLY;
