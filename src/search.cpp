...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool skipEarlyPruning);
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool skipEarlyPruning) {
        ...
        &&  pos.non_pawn_material(pos.side_to_move()))
    {
        ...
        Value nullValue = depth-R < ONE_PLY ? -qsearch<NonPV, false>(pos, ss+1, -beta, -beta+1)
                                            : - search<NonPV, !CutNode>(pos, ss+1, -beta, -beta+1, depth-R, true);
        pos.undo_null_move();
        ...
        if (nullValue >= beta)
        {
            ...
            Value v = depth-R < ONE_PLY ? qsearch<NonPV, false>(pos, ss, beta-1, beta)
                                        :  search<NonPV, false>(pos, ss, beta-1, beta, depth-R, true);
        ...
        &&  abs(beta) < VALUE_MATE_IN_MAX_PLY)
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (pos.legal(move))
            {
                ...
                pos.do_move(move, st);
                value = -search<NonPV, !CutNode>(pos, ss+1, -rbeta, -rbeta+1, rdepth, false);
                pos.undo_move(move);
                if (value >= rbeta)
                    return value;
            }
        ...
        && (PvNode || ss->staticEval + 256 >= beta))
    {
        ...
        search<NT, CutNode>(pos, ss, alpha, beta, d, true);
...
moves_loop: // When in check search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
          ...
          &&  pos.legal(move))
      {
          ...
          value = search<NonPV, CutNode>(pos, ss, rBeta - 1, rBeta, d, true);
          ...
          && (!captureOrPromotion || moveCountPruning))
      {
          ...
          value = -search<NonPV, true>(pos, ss+1, -(alpha+1), -alpha, d, false);
      ...
      if (doFullDepthSearch)
          value = newDepth <   ONE_PLY ?
                            givesCheck ? -qsearch<NonPV,  true>(pos, ss+1, -(alpha+1), -alpha)
                                       ...
                                       : - search<NonPV, !CutNode>(pos, ss+1, -(alpha+1), -alpha, newDepth, false);
